...
from app.llm import PROVIDERS, LLMResponseError
from app.llm_stub import StubLLM
from app.pdftext import PdfExtractionError
from app.routes_resumes import IMPORT_REVIEW_WARNING
from tests.conftest import SAMPLE_LATEX, RecordingStubLLM


# cdc1a82 deleted the per-request "mock" provider, so an import now names a
# real provider from ``app.llm.PROVIDERS`` and needs a stored key for it
# (app/auth.py:274-300). The offline model moved to ``app/llm_stub.StubLLM``
# and is injected as the client instead of being selected by provider name.
IMPORT_PROVIDER = "openai"
IMPORT_API_KEY = "sk-import-1234567890"
...
async def _store_provider_key(client, provider: str = IMPORT_PROVIDER) -> None:
    """Give the signed-in user a key for ``provider`` so imports are authorized."""

    response = await client.put(
        "/api/keys/{0}".format(provider), json={"api_key": IMPORT_API_KEY}
    )
    assert response.status_code == 200, response.text


def _disable_pdf_side_channels(app) -> None:
    """Silence the rasterizer/link extractor added by the ingestion upgrade.

    ``_read_pdf_upload`` now also rasterizes the upload and reads its link
    annotations (app/routes_resumes.py:329-355). Both are best-effort and would
    otherwise shell out to poppler with these fake PDF bytes, so these tests pin
    them to "nothing extra" and keep asserting on the text path alone.
    """

    app.state.services.pdf_renderer = lambda pdf_bytes: []
    app.state.services.pdf_link_extractor = lambda pdf_bytes: []


async def _import_resume(client, name: Optional[str] = None) -> dict:
    payload = {"latex": SAMPLE_LATEX, "provider": IMPORT_PROVIDER}
    if name is not None:
        payload["name"] = name
...
async def test_latex_import_creates_versioned_resume(
    ...
    app = make_app(llm=StubLLM())
    async with make_client(app) as client:
        await register_user(client)
        await _store_provider_key(client)
        ...
        # The offline-mock warning went away with the "mock" provider itself:
        # resolve_llm_selection returns no warnings now (app/auth.py:262-313),
        # so the import review notice is the only one left.
        assert any("review" in warning.lower() for warning in body["warnings"])
        assert body["warnings"] == [IMPORT_REVIEW_WARNING]
...
async def test_resumes_are_isolated_between_users(
    ...
    async with make_client(app) as owner, make_client(app) as intruder:
        ...
        await _store_provider_key(owner)
...
async def test_resume_quota_is_enforced(
    ...
    async with make_client(app) as client:
        ...
        second = await client.post(
            "/api/resumes", json={"latex": SAMPLE_LATEX, "provider": IMPORT_PROVIDER}
        )
...
async def test_new_versions_bump_and_expose_source(
    ...
    async with make_client(app) as client:
        ...
        version2 = await client.post(
            ...
            json={"latex": SAMPLE_LATEX + "% v2", "provider": IMPORT_PROVIDER},
        )
...
async def test_version_quota_is_enforced(
    ...
    async with make_client(app) as client:
        ...
        blocked = await client.post(
            "/api/resumes/{0}/versions".format(resume_id),
            json={"latex": SAMPLE_LATEX, "provider": IMPORT_PROVIDER},
        )
...
async def test_pdf_import_uses_extractor_and_text_extraction_prompt(
    ...
    app = make_app(pdf_extractor=extractor, llm=llm)
    _disable_pdf_side_channels(app)
    async with make_client(app) as client:
        await register_user(client)
        await _store_provider_key(client)
        response = await client.post(
            ...
            data={"provider": IMPORT_PROVIDER, "name": "From PDF"},
        )
        ...
        # The extracted text — not the LaTeX prompt — is what reached the model.
        assert llm.extract_calls == [
            {
                "source": PDF_TEXT,
                "provider": IMPORT_PROVIDER,
                "model": PROVIDERS[IMPORT_PROVIDER].default_model,
...
async def test_pdf_version_import_bumps_existing_resume(
    ...
    app = make_app(pdf_extractor=lambda pdf_bytes: PDF_TEXT, llm=StubLLM())
    _disable_pdf_side_channels(app)
    async with make_client(app) as client:
        await register_user(client)
        await _store_provider_key(client)
