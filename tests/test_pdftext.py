...
def _fake_run(
    ...
    Two things changed in the ingestion upgrade and are reflected here:
    * ``extract_pdf_text`` now runs a ``pdfinfo`` page-count probe before
      pdftotext (app/pdftext.py:226 -> ``_page_count``), so the fake answers
      that probe from ``pages`` and records only the pdftotext invocation --
      the command assertions below stay about the extraction call.
      ``pages=None`` makes the probe fail, which the extractor treats as
      "unknown, proceed" (app/pdftext.py:172-193).
    * Both call sites pass keyword arguments the old fake did not accept
      (``text=``/``errors=`` on the probe, app/pdftext.py:180-187), hence
      ``**kwargs``.
    """
...
def test_page_count_over_the_cap_is_rejected_before_extraction(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The pdfinfo page cap added in the ingestion upgrade short-circuits."""

    calls = _fake_run(monkeypatch, pages=DEFAULT_MAX_PAGES + 6)

    with pytest.raises(PdfExtractionError) as excinfo:
        extract_pdf_text(PDF_BYTES, max_pages=DEFAULT_MAX_PAGES)
    assert excinfo.value.code == "pdf_too_many_pages"
    assert calls == []  # pdftotext is never spawned for an oversized document


def test_unavailable_pdfinfo_fails_open_and_still_extracts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A page count that cannot be determined must not block the import."""

    calls = _fake_run(monkeypatch, pages=None)
