...
# cdc1a82 deleted the per-request "mock" provider: a tailor request now names a
# real provider from ``app.llm.PROVIDERS`` and needs a stored key for it
# (app/auth.py:274-300). The offline model is injected as the client instead.
TAILOR_PROVIDER = "openai"
TAILOR_API_KEY = "sk-tailor-1234567890"
...
async def _store_provider_key(
    client, provider: str = TAILOR_PROVIDER, api_key: str = TAILOR_API_KEY
) -> None:
    """Give the signed-in user a key for ``provider`` so LLM calls are authorized."""

    response = await client.put(
        "/api/keys/{0}".format(provider), json={"api_key": api_key}
    )
...
async def _setup_user_with_resume(client, register_user) -> str:
    ...
    imported = await client.post(
        "/api/resumes", json={"latex": SAMPLE_LATEX, "provider": TAILOR_PROVIDER}
    )
...
async def test_multi_user_tailor_persists_run_with_saved_jd(
    ...
    async with make_client(app) as client:
        ...
        # so the skipped compile is the only warning this run carries.
        assert body["warnings"] == ["Compilation was skipped by request."]
...
async def test_tailor_multi_user_error_paths(
    ...
    async with make_client(app) as client:
        ...
        no_resume = await client.post(
            "/api/tailor",
            json={"job_description": JD_CONTENT, "provider": TAILOR_PROVIDER},
        )
