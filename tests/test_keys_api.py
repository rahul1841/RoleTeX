...
def _mock_resume_proposal() -> TailorProposal:
    """A proposal valid against the deterministic stub-extraction resume."""
...
async def test_providers_catalog_is_public_and_curated(make_app, make_client) -> None:
    ...
    async with make_client(app) as client:
        ...
        # Curated: the operator-only "custom" passthrough is never offered, and
        # neither is an offline provider. cdc1a82 removed the keyless "mock"
        # entry from the product; the offline path now lives in app/llm_stub.py
        # and is opt-in via LLM_PROVIDER=stub, so it is not in PROVIDERS and
        # must not appear in the catalog either.
        assert "custom" not in providers
        assert "mock" not in providers
        assert "stub" not in providers
        # Every catalogued provider is a real one the user must supply a key for.
        assert all(item["needs_key"] is True for item in providers.values())
...
async def test_stored_key_round_trip_reaches_llm_decrypted(
    ...
    async with make_client(app) as client:
        ...
        # The import is only scaffolding for the tailor call below. It used to
        # run on the keyless "mock" provider, which no longer exists, so it now
        # runs on the same stored-key provider as the tailor step.
        imported = await client.post(
            "/api/resumes", json={"latex": SAMPLE_LATEX, "provider": "groq"}
        )
