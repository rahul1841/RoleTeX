...
def test_clamp_headline_enforces_word_and_char_limits() -> None:
    ...
    # Intentional change (app/importer.py:27-39): a blank summary no longer
    # falls back to the invented "Professional summary" headline, it stays
    # empty. \ResumeContact drops the whole headline line when it is empty
    # (app/importer.py:251-256), and ResumeData.summary defaults to ""
    # (app/schemas.py:125), so nothing downstream needs a placeholder.
    assert clamp_headline("   ") == ""
    assert clamp_headline("") == ""
    assert len(clamp_headline("x" * 400)) <= 120


@pytest.mark.parametrize(
    ("raw", "expected_font", "expected_accent"),
    [
        # Intentional change (app/importer.py:20-22, 64-66): paper is no longer
        # a user choice, so even a previously-whitelisted "letterpaper" hint is
        # now forced to A4. Every other value is still whitelisted as before.
        ({"paper": "letterpaper", "font_size": "12pt", "accent_hex": "#1a2B3c"}, "12pt", "1A2B3C"),
        ({"paper": "junk", "font_size": "99pt", "accent_hex": "nothex"}, "10pt", None),
        ({}, "10pt", None),
    ],
)
def test_sanitize_style_whitelists_every_value(raw, expected_font, expected_accent) -> None:
    style = sanitize_style(raw)
    assert style.paper == PAPER == "a4paper"
...
def test_assembled_template_is_valid_and_reflects_style() -> None:
    ...
    # The requested "letterpaper" is ignored: assemble_template always emits the
    # fixed PAPER constant (app/importer.py:351). font_size and the accent are
    # still the caller's, so the template still reflects the bounded style.
    assert r"\documentclass[a4paper,12pt]{article}" in template
    assert "letterpaper" not in template
